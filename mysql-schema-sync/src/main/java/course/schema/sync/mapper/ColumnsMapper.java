...
public interface ColumnsMapper extends BaseMapper {
    ...
    @Select("SELECT `table_schema`,`table_name`,`column_name`,`column_type`,`data_type`,`column_default`,`is_nullable` FROM `information_schema`.`columns` WHERE `table_schema` = #{tableSchema} AND `table_name` = #{tableName}")
